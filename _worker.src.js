...
function surge(content, url) {
	...
	for (let x of 每行内容) {
		if (x.includes('= trojan,')) {
			...
			const 正确内容 = `skip-cert-verify=true, ws=true, ws-path=/?ed=2560, ws-headers=Host:"${host}", tfo=true, udp-relay=false, test-url=http://edge.microsoft.com/captiveportal/generate_204`;
